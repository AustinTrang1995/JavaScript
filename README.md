...
**Note:** 
The reason these _functions_ are in a separate _module_ from `salaryData.js` is to achieve separation of concerns. 

`salaryData.js` is concerned only with providing access to _raw data_ while `workAroundModule.js` is concerned with digging into the _data_ to find precise values.
...
**6.** Each of the incomplete _functions_ in `workAroundModule.js` contains an _empty array_ (`[]`) that needs to be replaced.

We will need to use the appropriate imported _data/functions_ from the `salaryData.js` _module_ to replace these _arrays_.
